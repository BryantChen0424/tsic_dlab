...
int main(int argc, char** argv) {
    ...
    for (int case_id = 0; case_id < num_cases; ++case_id) {
        std::vector<int> w_timings;
        std::vector<int> w_datas;
        int val;
        {
            ...
            while (iss >> val) w_timings.push_back(val);
        }
        {
            while (infile.peek() == ' ' || infile.peek() == '\n') infile.get();
            std::string line;
            std::getline(infile, line);
            std::istringstream iss(line);
            while (iss >> val) w_datas.push_back(val);
        }
        ...
        int idx = 0;
        int local_t0 = t;
        int time_limit = t + (local_t0 + w_timings.back() * PERIOD + PERIOD * 20);
        ...
        while (t < time_limit) {
            ...
            dut->clk = 0;
            dut->w_valid = (w_timings.size() > 0) && ((idx < w_timings.size() && t == w_timings[idx] * PERIOD + local_t0)) ? 1 : 0;
            dut->w_data  = (w_timings.size() > 0) && ((idx < w_timings.size() && t == w_timings[idx] * PERIOD + local_t0)) ? w_datas[idx] : 0;
            dut->eval();
            tfp->dump(t += PERIOD / 2);
        ...
        for (int i = 0; i < std::min(ARR_SIZE, int(w_datas.size())); i++) {
            if (dut->array->R[i] != w_datas[i]) {
                local_fail = true;
                fail_reason << "data error, R[" << i << "]=" << (int)dut->array->R[i] << ", "
                            << "expected=" << w_datas[i]
                            << std::endl;
            }
        }
