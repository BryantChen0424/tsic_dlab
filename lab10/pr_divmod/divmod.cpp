...
int main(int argc, char** argv) {
    ...
    for (int case_id = 0; case_id < num_cases; ++case_id) {
        int a, b, golden_remainder, golden_quotient;
        a = cases[case_id].first;
        b = cases[case_id].second;
        golden_remainder = a % b;
        golden_quotient = int(floor(a / b));
        int local_t0 = t;
        ...
        while (t < time_limit && leave_case_count_down > 0) {
            ...
            uint8_t dut_quotient = dut->quotient;
            ...
            if (dut_ready && has_ready) {
                ...
                pass_the_cycle = (dut_remainder == golden_remainder) && (dut_quotient == golden_quotient);
            }
            else {
                pass_the_cycle = (dut_remainder == 0) && (dut_quotient == 0);
            }

            if (!pass_the_cycle) {
                if (dut_ready && has_ready)
                    fail_reason << "[cycle " << (t / PERIOD) << "], ready=1, quotient=" << (int)dut_quotient << ", remainder=" << (int)dut_remainder << ", expected: " << dut_quotient << ", " << golden_remainder << std::endl;
                else if (dut_ready && !has_ready)
                    fail_reason << "[cycle " << (t / PERIOD) << "], assert too many ready" << golden_remainder << std::endl;
                else
                    fail_reason << "[cycle " << (t / PERIOD) << "], ready=0, quotient=" << (int)dut_quotient << ", remainder=" << (int)dut_remainder << ", expected: " << 0 << ", " << 0 << std::endl;
            }
