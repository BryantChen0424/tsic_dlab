#include "Vfifo.h"
#include "Vfifo__Syms.h"
#include "verilated.h"
#include "verilated_vcd_c.h"
#include <iostream>
#include <fstream>
#include <sstream>
#include <vector>
#include <bitset>
#include <iomanip>
#include <thread>
#include <chrono>
#include <cmath>
#include <algorithm>
#include <queue>

#define PERIOD 10
#define ARR_SIZE 8

template<int W>
std::string bin(unsigned val) {
    return std::bitset<W>(val).to_string();
}

int main(int argc, char** argv) {
    Verilated::commandArgs(argc, argv);
    Verilated::traceEverOn(true);
    Vfifo* dut = new Vfifo;
    VerilatedVcdC* tfp = new VerilatedVcdC;
    dut->trace(tfp, 99);
    tfp->open("wave.vcd");

    std::ifstream infile("input.data");
    std::ofstream log("log.txt");
    std::ofstream result("result.txt");

    log << "##SEC_STUDENT_CAN_SEE" << std::endl;
    std::cout << "##SEC_STUDENT_CAN_SEE" << std::endl;

    int num_cases;
    infile >> num_cases;

    int t = 0;
    bool global_fail = false;

    for (int case_id = 0; case_id < num_cases; ++case_id) {
        std::vector<int> timings;
        std::vector<int> datas;
        int val;
        {
            while (infile.peek() == ' ' || infile.peek() == '\n') infile.get();
            std::string line;
            std::getline(infile, line);
            std::istringstream iss(line);
            while (iss >> val) timings.push_back(val);
        }
        {
            while (infile.peek() == ' ' || infile.peek() == '\n') infile.get();
            std::string line;
            std::getline(infile, line);
            std::istringstream iss(line);
            while (iss >> val) datas.push_back(val);
        }

        // for (auto iter: w_datas) std::cout << iter << ", ";
        // std::cout << std::endl;
        

        // Reset
        dut->clk = 0; dut->rst_n = 0; dut->eval(); tfp->dump(t += PERIOD / 2);
        dut->clk = 0; dut->rst_n = 0; dut->eval(); tfp->dump(t += PERIOD / 2);
        dut->clk = 0; dut->rst_n = 0; dut->eval(); tfp->dump(t += PERIOD / 2);
        dut->clk = 1; dut->rst_n = 0; dut->eval(); tfp->dump(t += PERIOD / 2);
        dut->clk = 0; dut->rst_n = 0; dut->eval(); tfp->dump(t += PERIOD / 2);
        dut->clk = 1; dut->rst_n = 0; dut->eval(); tfp->dump(t += PERIOD / 2);
        dut->clk = 0; dut->rst_n = 1; dut->eval(); tfp->dump(t += PERIOD / 2);
        dut->clk = 1; dut->rst_n = 1; dut->eval(); tfp->dump(t += PERIOD / 2);

        // Simulate
        int idx = 0;
        int local_t0 = t;
        int time_limit = t + (local_t0 + timings.back() * PERIOD + PERIOD * 20);
        bool local_fail = false;
        std::ostringstream fail_reason;
        std::queue<int> golden_fifo;

        while (t < time_limit) {
            // Falling edge assign input
            bool req = (timings.size() > 0) && ((idx < timings.size() && t == timings[idx] * PERIOD + local_t0));
            bool gen_r = req && datas[idx] < 0;
            bool gen_w = req && datas[idx] >= 0;
            dut->clk = 0;
            dut->w_valid = gen_w ? 1 : 0;
            dut->w_data  = gen_w ? datas[idx] : 0;
            dut->r_valid = gen_r ? 1 : 0;
            dut->eval();
            tfp->dump(t += PERIOD / 2);
            
            // Verification
            bool dut_w_ready = dut->w_ready;
            bool dut_r_ready = dut->r_ready;
            bool pass_the_cycle = (dut_w_ready && idx < ARR_SIZE) || (!dut_w_ready && idx >= ARR_SIZE);

            if (!pass_the_cycle) {
                local_fail = true;
                fail_reason << "[cycle " << (t / PERIOD) << "] "
                            << "write num=" << idx << ", "
                            << "w_ready=" << dut_w_ready << ", "
                            << "expected=" << (idx > ARR_SIZE)
                            << std::endl;
            }

            // Rising edge assign input
            dut->clk = 1;
            dut->eval();
            tfp->dump(t += PERIOD / 2);
            
            // Generate golden
            if (gen_w) {
                golden_fifo.push(dut->w_data);
            }
            if (dut->w_valid == 1) {
                ++idx;
            }

            //
            if (!pass_the_cycle) {
                break;
            }
        }

        for (int i = 0; i < std::min(ARR_SIZE, int(w_datas.size())); i++) {
            if (dut->fifo->R[i] != w_datas[i]) {
                local_fail = true;
                fail_reason << "data error, R[" << i << "]=" << (int)dut->fifo->R[i] << ", "
                            << "expected=" << w_datas[i]
                            << std::endl;
            }
        }
        
        if (local_fail) {
            for (auto iter: w_datas) std::cout << iter << ", ";
            std::cout << std::endl;
        }

        dut->clk = 0; dut->rst_n = 0; dut->eval(); tfp->dump(t += PERIOD / 2);


        if (local_fail) {
            global_fail = true;
            std::cout << "Test case " << case_id + 1 << ": fail" << std::endl;
            std::cout << fail_reason.str() << std::endl;
            log << "Test case " << case_id + 1 << ": fail" << std::endl;
            log << fail_reason.str() << std::endl;
        } else {
            std::cout << "Test case " << case_id + 1 << ": PASS" << std::endl;
            log << "Test case " << case_id + 1 << ": PASS" << std::endl;
        }
        tfp->dump(t += PERIOD);
        tfp->dump(t += PERIOD);
        tfp->dump(t += PERIOD);
        tfp->dump(t += PERIOD);
    }

    log << "##END_STUDENT_CAN_SEE" << std::endl;
    std::cout << "##END_STUDENT_CAN_SEE" << std::endl;

    result << (global_fail ? "fail" : "pass") << std::endl;
    if (!global_fail) std::cout << "congratulation" << std::endl;

    tfp->close();
    log.close();
    result.close();
    delete dut;
    delete tfp;
    std::this_thread::sleep_for(std::chrono::milliseconds(300));
    return 0;
}